...
\emph{Asymmetric}. These cryptography techniques rely heavily on the hardness of solving problems such as Inetger Factorization and the Discrete 
Logarithm problem.
...
The inherent security of symmetric cryptography is based on the length of the keys used for encryption/decryption. A 128 bit key length would take billions of years
to crack on classical hardware. 256 bit key lengths are thought to be theoreticallly resistant to Quantum attacks. Symmetric Cryptography allows for fairly high security with fast speeds,
...
A hash is of the data to be signed is generated, this hash is then encrypted using the private key.
\subsection{Security}
How secure they are currently
